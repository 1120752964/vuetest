...
<!--        </el-select>-->
<!--      </el-form-item>-->
      <el-form-item label="账号"  prop="staff">
        <el-input  v-model="user.unumber" style="width: 230px;" autocomplete="off"></el-input>
      </el-form-item>
      <el-form-item label="密码"   prop="upassword">
        <el-input type="password" v-model="user.upassword" style="width: 230px;" autocomplete="off"></el-input>
      </el-form-item>
      <el-form-item>
    ...
    login () {
      let _this = this;
        ...
        this.axios({
          ...
          if (res.data!=="error"){
            axios.get('http://localhost:8181/user/findUserByUnumber/'+_this.user.unumber).then(function (resp){
              _this.staff = resp.data;
            localStorage.clear();
            console.log(res.data);
            localStorage.setItem('userUnumber',res.data);
            // localStorage.setItem('stnumber',res.data.stnumber);
            // localStorage.setItem('sttype',res.data.sttype);
            _this.$router.push('/ClothingMall');
            })
          }else {
            alert('账号或密码错误');
          }
