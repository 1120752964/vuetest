    ...
    <div class="form" style="width:400px;height: 350px;z-index: 1;position: absolute;left: 70%;margin-top:200px;margin-right:80px;background-color: rgb(255,255,255)">
      ...
      <el-form  status-icon :rules="rules" ref="ruleForm"  label-width="100px" class="demo-ruleForm">
        ...
        <el-form-item label="账号"  prop="staff">
          <el-input  v-model="administrator.anumber" style="width: 230px;" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="密码"   prop="upassword">
          <el-input type="password" v-model="administrator.apassword" style="width: 230px;" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item>
...
export default {
  ...
  methods: {
    ...
    login () {
      ...
      this.axios({
        method: 'post',
        url: 'http://localhost:8181/administrator/login',
        data: _this.administrator
      }).then(res => {
        console.log(res.data);
        _this.administrator=res.data
        if (res.data!=="error"){
            localStorage.clear();
            localStorage.setItem('aid',_this.administrator.aid);
