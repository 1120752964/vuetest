...
export default {
  ...
  created(){
    if(this.$route.query.cid!=null) {
      const _this = this
      axios.get('http://localhost:8181/commodity/findCommodityByCid/' + this.$route.query.cid).then(function (resp) {
        console.log(resp.data)
        _this.ruleForm = resp.data
      })
      axios.get('http://localhost:8181/commodity/findCommodityDetailByCid/' + this.$route.query.cid).then(function (resp) {
        console.log(resp.data)
        _this.detailFrom.domains = resp.data
      })
    }
  }
}
